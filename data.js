    ...
    "models": [
      {
        ...
        "audit_backbone": "In-house histology vision encoder (100M images) + pretrained LLM",
        "audit_architecture": "Vision–language generalist: vision encoder fused with an LLM via a multimodal projector; handles interleaved image+text",
        "audit_tasks": "Multiple-choice diagnosis, open-ended pathology Q&A, education & research",
        "audit_domain": "Disease-agnostic; diverse tissue origins",
        "audit_benchmark": "1.18M image–caption pairs; 250K+ visual-language instructions",
        "audit_paradigm": "Vision-language pretraining + instruction fine-tuning",
        "audit_result": "87% MCQ diagnostic accuracy with clinical context; preferred over GPT-4V",
        "hf": "",
      ...
      },
      {
        ...
        "audit_backbone": "CONCH patch encoder + slide-level encoder + LLM",
        "audit_architecture": "WSI → patch embeddings (CONCH) → slide encoder → multimodal projector → LLM",
        "audit_tasks": "Slide-level captioning, VQA, description, reasoning",
        "audit_domain": "10 cancer types (TCGA/BCNB)",
        "audit_benchmark": "SlideInstruction (4.2K captions, 176K VQA); SlideBench",
        "audit_paradigm": "Instruction tuning on WSI data",
        "audit_result": "SOTA on 18/22 tasks; 81.17% SlideBench-VQA (TCGA)",
      ...
      },
      {
        ...
        "audit_backbone": "Mixture-of-experts MLLM",
        "audit_architecture": "Unified ROI- and WSI-level model; multiscale + multitask routing via MoE",
        "audit_tasks": "ROI classification/detection/segmentation/VQA, WSI classification/VQA (72 tasks)",
        "audit_benchmark": "2.3M ROI + 188K WSI instruction samples",
        "audit_paradigm": "Scale-dependent SFT + task-aware RL (no CoT supervision)",
        "audit_result": "Validated across 72 tasks; reasoning-enhanced SOTA",
      ...
      },
      {
        ...
        "audit_backbone": "Patho-R1-7B base + LLM-enhanced router",
        "audit_architecture": "RL- and router-enhanced copilot that selects the best capability per request",
        "audit_tasks": "Expert-level diagnosis, patch summarization, cross-modality (transcriptomic) generation",
        "audit_domain": "Histopathology (multimodal)",
        "audit_paradigm": "RL (GRPO) fine-tuning + routing",
        "audit_result": "Router >80% accuracy; human eval with Yale pathologists; corrects expert reasoning",
  ...
  },
  {
    ...
    "models": [
      ...
      },
      {
        ...
        "audit_backbone": "Multimodal VLM; Patho-CLIP encoder",
        "audit_architecture": "Three-stage pipeline: continued pretraining → CoT SFT → RL refinement",
        "audit_tasks": "Zero-shot classification, cross-modal retrieval, VQA, MCQ",
        "audit_benchmark": "3.5M image–text pairs; 500K CoT samples (textbooks + experts)",
        "audit_paradigm": "Pretrain + SFT + RL (GRPO + DAPO)",
        "audit_result": "Robust across pathology tasks; releases Patho-CLIP + Patho-R1 (3B/7B)",
      ...
      },
      {
        ...
        "audit_backbone": "Single model unifying patch/region/WSI capabilities",
        "audit_architecture": "Agent autonomously navigates WSI (zoom-in/out, move) from observed features; emits interpretable summaries",
        "audit_tasks": "Interpretable diagnosis at patch, region and slide scale",
        "audit_benchmark": "PathMMU-HR2 (first expert-validated large-region benchmark)",
        "audit_paradigm": "Multi-stage training across three scales",
        "audit_result": "Outperforms prior models across benchmarks at three image scales (NeurIPS 2025)",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_backbone": "Pathology-o3 two-stage agent (backbone-agnostic)",
        "audit_architecture": "AI Session Recorder logs expert viewing → behavioral commands + boxes → propose ROIs then behavior-guided reasoning",
        "audit_tasks": "ROI proposal + explainable metastasis diagnosis",
        "audit_domain": "Gastrointestinal lymph-node metastasis",
        "audit_benchmark": "Pathology-CoT dataset from viewer logs; Stanford internal + Sweden external",
        "audit_paradigm": "Behavior-grounded supervision from expert navigation",
        "audit_result": "100% recall (Stanford), 97.6% (Sweden); exceeds OpenAI o3",
      ...
      },
      {
        ...
        "audit_backbone": "Off-the-shelf LLM + pathology models (training-free)",
        "audit_architecture": "Navigator (locate micro-regions) → Perceptor (morphology cues) → Executor (evolving NL chain-of-thought)",
        "audit_tasks": "Open-ended & constrained WSI VQA",
        "audit_benchmark": "Five datasets",
        "audit_paradigm": "Training-free",
        "audit_result": "Strong zero-shot; surpasses task-specific baselines (ECCV 2026)",
      ...
      },
      {
        ...
        "audit_backbone": "Multimodal reasoning agent (GPT-4o as comparison)",
        "audit_architecture": "Iterative sample → self-reflect → reason → propose new ROIs, building a reasoning chain",
        "audit_tasks": "Subtyping, longitudinal analysis, report generation",
        "audit_domain": "Tumor microenvironment; breast-cancer reporting",
        "audit_paradigm": "Iterative reasoning; no dense pixel annotations",
        "audit_result": "+6.7%/+3.1% AUROC (subtyping/longitudinal); +10% over GPT-4o on breast reports",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_backbone": "Pathology visual FM + VLM + RL-trained reasoning model",
        "audit_architecture": "Evidence-seeking inference: initial diagnosis → evidence-seeking → final decision",
        "audit_tasks": "Proactive diagnosis refinement across clinical scenarios",
        "audit_paradigm": "RL-trained reasoning; proactive evidence acquisition",
        "audit_result": "SOTA diagnostic accuracy; recovers subtle nuclear features & local invasions",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_backbone": "Instruction-tuned LLM + scale-specific projectors",
        "audit_architecture": "Hierarchical alignment cell=word, patch=phrase, region=sentence, WSI=paragraph; Cell-Cell Attention Fusion; hierarchical contrastive + cross-scale consistency",
        "audit_tasks": "Open-ended reasoning, VQA, report & caption generation",
        "audit_benchmark": "13 WSI-level benchmarks across 6 CPath tasks",
        "audit_paradigm": "Three-stage training",
        "audit_result": "New SOTA on 13 WSI-level benchmarks",
    ...
    "models": [
      {
        ...
        "audit_backbone": "GPT-5 (general-purpose LMM); training-free",
        "audit_architecture": "LMM iteratively selects multi-magnification crops and aggregates evidence over time",
        "audit_tasks": "WSI question answering",
        "audit_benchmark": "MultiPathQA (934 questions, 868 WSIs; 128 pathologist-authored)",
        "audit_paradigm": "Training-free",
        "audit_result": "SOTA on 4/5 benchmarks; beats pathology-specialized models",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_architecture": "Cross-Magnification navigation Tool (CMT) aggregates adjacent-magnification context; Magnification Selection Tool (MST) uses memory-driven reasoning",
        "audit_tools": "CMT, MST",
        "audit_tasks": "Multi-magnification WSI diagnosis with adaptive scale selection",
        "audit_paradigm": "Agent framework (memory-driven)",
        "audit_result": "+1.45% AUC, +2.93% BACC over non-agent baseline",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_backbone": "Frozen pathology features + PLIP + perceptor–adjudicator (training-free)",
        "audit_architecture": "Scan (surprise field via shared online memory) → Search (question-conditioned PLIP within abnormal pool) → Readout",
        "audit_tasks": "WSI-VQA",
        "audit_benchmark": "WSI-VQA, SlideBench-BCNB",
        "audit_paradigm": "Training-free",
        "audit_result": "Higher accuracy + more interpretable evidence trajectories",
      ...
      },
      {
        ...
        "audit_backbone": "Off-the-shelf foundation models (training-free)",
        "audit_architecture": "Bayesian belief over competing diagnoses; acquire patches by max expected information gain; evidence controller (answer / acquire / zoom)",
        "audit_tasks": "Uncertainty-aware WSI-VQA",
        "audit_benchmark": "Five WSI-VQA benchmarks",
        "audit_paradigm": "Training-free; Bayesian evidence acquisition",
        "audit_result": "Best among training-free agentic frameworks; more efficient evidence acquisition",
      ...
      },
      {
        ...
        "audit_backbone": "Pathology-trained Navigator (PPO) + Interpreter/Deliberator/Arbiter",
        "audit_architecture": "Navigator (abnormality-driven, PPO) → Morphology Interpreter → Deliberator → Arbiter",
        "audit_tasks": "WSI & region VQA; cancer subtype classification",
        "audit_domain": "Six TCGA cohorts",
        "audit_paradigm": "RL (PPO) active perception; pathologist-reviewed labels",
        "audit_result": "SOTA zero-shot; 80.14% subtyping (6 TCGA); pathologists reach 82.9% using its ROIs",
    ...
    "models": [
      {
        ...
        "audit_architecture": "Four collaborating agents: Triage → Navigation → Description → Diagnosis",
        "audit_tasks": "Melanoma diagnosis with natural-language explanations",
        "audit_domain": "Skin melanoma",
        "audit_paradigm": "Multi-agent orchestration",
        "audit_result": "+8% over SOTA; first AI to beat average pathologists by 9% (ICCV 2025)",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_backbone": "PathChat+ MLLM (1M+ instructions, ~5.5M QA turns)",
        "audit_architecture": "SlideSeek: multi-agent supervisor–explorer performing iterative hierarchical diagnostic reasoning",
        "audit_tasks": "Differential diagnosis; grounded summary reports",
        "audit_benchmark": "DDxBench (open-ended differential diagnosis)",
        "audit_result": "PathChat+ beats PathChat & SOTA; high DDxBench accuracy",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_architecture": "Task allocation (model zoo of patch/WSI MLLMs) + verification (consistency + knowledge base) + summary (visual maps)",
        "audit_tools": "Model zoo of patch & WSI MLLMs; pathology knowledge bases; domain models",
        "audit_tasks": "Multi-task WSI analysis",
        "audit_paradigm": "Collaborative multi-agent",
        "audit_result": "Beats WSI MLLMs and medical agent frameworks (MICCAI 2025)",
      ...
      },
      {
        ...
        "audit_backbone": "Agentic system (TissueLab) vs GPT-5",
        "audit_architecture": "Plans explainable workflows; tool factories across pathology/radiology/spatial-omics; real-time expert-in-the-loop refinement",
        "audit_tools": "Pathology, radiology & spatial-omics tool factories",
        "audit_tasks": "Staging, prognosis, treatment-planning quantifications",
        "audit_paradigm": "Co-evolving with active learning from clinicians",
        "audit_result": "SOTA vs end-to-end VLMs and GPT-5; adapts to unseen diseases within minutes",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_backbone": "CodeAct agent (smolagents) that writes & runs Python",
        "audit_architecture": "Iteratively generates and executes Python; can create new tools ad hoc",
        "audit_tools": "49 domain tools (nuclei segmentation, WSI encoding, …)",
        "audit_tasks": "Data processing, quantitative analysis, hypothesis testing",
        "audit_benchmark": "SlideQuest (90 pathologist-verified questions)",
        "audit_paradigm": "Code-generating agent",
        "audit_result": "Beats coding-agent baselines; links morphology to PAM50 subtypes",
      ...
      },
      {
        ...
        "audit_architecture": "Stage 1 WSI-Gene CoT case bank (low-mag screening, cross-modal & confidence-aware patch mining, gene-stratified analysis); Stage 2 dichotomy-based multi-expert inference with RAG",
        "audit_tasks": "Multimodal survival prediction",
        "audit_domain": "Five TCGA cohorts",
        "audit_paradigm": "Hierarchical CoT + experiential case bank + RAG",
        "audit_result": "Beats conventional methods, proprietary MLLMs and medical agents",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_architecture": "Diagnosis decomposed into micro-tasks per specialized agent; gene & table agents use knowledge enhancement; RL refines tool selection/assignment",
        "audit_tasks": "Automated GAPP scoring, genotype risk alerts, multimodal reporting",
        "audit_domain": "Pheochromocytomas & paragangliomas (PPGL)",
        "audit_paradigm": "Agentic swarm + RL for tool selection",
        "audit_result": "Auditable reasoning trail; quantified cellularity & Ki-67; genotype risk (e.g., SDHB)",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_backbone": "LLM (report standardization) + CTVisionNet + MacroVisionNet; MATCH-Net fusion",
        "audit_architecture": "Routing engine dispatches report standardization, CT segmentation, WSI analysis, phenotype mapping and prognosis; multi-head-attention fusion",
        "audit_tools": "CTVisionNet, MacroVisionNet, interactive CT segmentation",
        "audit_tasks": "Prognosis / survival-risk stratification",
        "audit_domain": "Bladder urothelial carcinoma",
        "audit_benchmark": "1,185 patients across 4 centers",
        "audit_result": "C-index 0.836–0.874; stratifies high/low risk; flags adjuvant-chemo candidates",
      ...
      },
      {
        ...
        "audit_backbone": "Frozen foundation-model features; LLM-backend-agnostic",
        "audit_architecture": "Two tiers: specialist agents (pathology/molecular/clinical) → governance agent applies executable rules + Guideline Knowledge Graph (hard/soft paths)",
        "audit_tasks": "Risk stratification with mandatory guideline overrides (e.g., POLE)",
        "audit_domain": "Endometrial cancer",
        "audit_benchmark": "TCGA-UCEC (n=541); CPTAC-UCEC (n=95)",
        "audit_result": "0.943 acc / 0.973 macro AUC (TCGA); 0.842 (CPTAC); 0.93% logic-violation rate",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_architecture": "Collaborative multi-agent: input understanding → evidence retrieval → filtering → diagnosis, grounded in a pathology hypergraph",
        "audit_tools": "Pathology hypergraph (4.55M entities, 7.10M relations); 110,472-doc evidence corpus",
        "audit_tasks": "Text, ROI and gigapixel-WSI diagnostics",
        "audit_benchmark": ">200,000 real-world cases",
        "audit_result": "SOTA; improves pathologist accuracy & confidence (user studies)",
        "github": "",
        "hf": "",
    ...
    "models": [
      {
        ...
        "audit_backbone": "Pathology VLM trained with RL",
        "audit_architecture": "Multimodal RAG over page-level textbook embeddings; joint text–image search; reasoning, task decomposition, multi-turn",
        "audit_tools": "Multimodal textbook retrieval database",
        "audit_tasks": "Multiple-choice diagnosis, VQA",
        "audit_paradigm": "RL-trained agentic RAG",
        "audit_result": "Outperforms existing multimodal models on complex pathology tasks (AAAI 2026)",
      ...
      },
      {
        ...
        "audit_architecture": "Extract interpretable cellular features → Feature Reasoning Agents compose & execute SQL over feature tables → Knowledge Comparison Agent checks vs pathology knowledge",
        "audit_tools": "SQL over cellular-feature tables; pathology knowledge base",
        "audit_tasks": "Auditable pathology VQA",
        "audit_benchmark": "Two pathology VQA datasets",
        "audit_paradigm": "Neuro-symbolic; executable SQL traces",
        "audit_result": "Improves interpretability & decision traceability with executable SQL traces",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_backbone": "LVLM planner + component agents",
        "audit_architecture": "Tool-centric bottom-up: domain tools → component agents → top-level planner; Atomic Execution Nodes build reasoning trajectories",
        "audit_tools": "Customized domain-adaptive pathology tools; spatial transcriptomics",
        "audit_tasks": "Molecularly informed pathology analysis",
        "audit_paradigm": "Trajectory-aware fine-tuning",
        "audit_result": "More robust inference and adaptive toolset use (ICLR 2026)",
      ...
      },
      {
        ...
        "audit_backbone": "GPT-4o planner + Digepath encoder (353M patches / 210K slides)",
        "audit_architecture": "DigeTools pipeline: feature extraction → tumor detection → subtype ID → attention-based ROI Finder → DigeCaption report (multi-turn)",
        "audit_tools": "DigeTools library (encoder, ROI Finder, DigeCaption)",
        "audit_tasks": "Diagnosis, molecular profiling, survival, report generation",
        "audit_domain": "Gastrointestinal pathology",
        "audit_benchmark": "SOTA on 32/33 GI tasks; 9-center early-cancer screening trial",
        "audit_result": "32/33 SOTA; deployed screening module 99.6% sensitivity / 85% specificity",
      ...
      },
      {
        ...
        "audit_architecture": "H&E diagnosis → self-confidence estimate → conditionally invoke virtual-staining/IHC tool → integrate H&E + virtual-IHC evidence",
        "audit_tools": "Virtual-staining / IHC tool",
        "audit_tasks": "Reliability-aware diagnosis",
        "audit_paradigm": "Confidence-gated tool use",
        "paper": ""
      },
      {
        "name": "PathoSage",
        ...
        "audit_architecture": "Three stages: knowledge retrieval → evidence collection → Structured Evidence Deliberation (conflict analysis in a fresh context)",
        "audit_tools": "Multiple pathology tools + knowledge sources; Beta-Bernoulli tool-reliability priors",
        "audit_tasks": "Patch-level VQA & classification",
        "audit_paradigm": "Training-free experience system (continuous credit assignment)",
        "audit_result": "Cuts VQA hallucination & classifier disagreement; beats MLLM & agentic baselines",
        "github": "",
        "hf": "",
    ...
    "models": [
      {
        ...
        "audit_architecture": "Multi-agent: knowledge-graph-anchored hypothesis generation → debate-based novelty assessment → automated validation on multimodal data",
        "audit_tools": "Ontology/knowledge graph; multimodal pathology datasets",
        "audit_tasks": "Interpretable image-based biomarker discovery",
        "audit_paradigm": "Multi-agent hypothesis generation & evaluation",
        "audit_result": "Turns biomarker discovery into a traceable, inspectable reasoning process",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_backbone": "LLM-driven agent + deep-learning multiplex-marker inference",
        "audit_architecture": "LLMs orchestrate biomarker inference and spatiotemporal reasoning over routine H&E (in-silico immunofluorescence)",
        "audit_tools": "In-silico multiplex immunofluorescence",
        "audit_tasks": "Spatiotemporal immune-dynamics reconstruction",
        "audit_domain": "Pancreatic cancer (PanIN progression)",
        "audit_benchmark": "~10.44M single-cell profiles",
        "audit_result": "Recovers immune-surveillance → exhaustion → stromal-dominant trajectory",
      ...
      },
      {
        ...
        "audit_backbone": "Interconnected LLM agents (no additional training)",
        "audit_architecture": "SPARK (System of Pathology Agents for Research and Knowledge): agents use language as a universal interface to reason, generate & implement biological hypotheses as analytical tools",
        "audit_tools": "Connects specialized algorithms; generates analytical tools from hypotheses",
        "audit_tasks": "Autonomous hypothesis generation; diagnostic/prognostic/predictive cellular parameters",
        "audit_domain": "5 cancer types (lung adeno/squamous, colorectal, breast, oropharyngeal); 18 cohorts, 5,400+ patients",
        "audit_paradigm": "Training-free agentic (no retraining)",
        "audit_result": "Reproduces pathology reasoning; produces biologically meaningful prognostic parameters (Nature Medicine)",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_architecture": "Translates NL research objectives into validated workflows via reusable skills (preprocessing, model development, evaluation, interpretation); rejects incompatible specs before execution",
        "audit_tools": "Library of domain-specific skills/tools",
        "audit_tasks": "ROI classification, WSI classification, segmentation, survival prediction",
        "audit_benchmark": "12 public datasets across 4 task families",
        "audit_paradigm": "Autonomous workflow agent",
        "audit_result": "Non-inferior to expert implementations; enables non-programmers to run studies",
        "github": "",
        "hf": "",
      ...
      },
      {
        ...
        "audit_architecture": "CodeAct-style Perception–Action loop: FM perception + Knowledge Fabric → hypothesis → generate & execute statistics in a sandbox → Cox survival evaluation → revise",
        "audit_tools": "Multimodal pathology FMs; biological Knowledge Fabric; sandboxed code execution",
        "audit_tasks": "Prognostic discovery (morphomolecular signatures)",
        "audit_domain": "Pan-cancer (TCGA)",
        "audit_paradigm": "CodeAct agent",
        "audit_result": "Generates & tests prognostic signatures across pan-cancer TCGA cohorts",
    ...
    "models": [
      {
        ...
        "audit_backbone": "In-context LLMs (Claude 4.0 best) + CONCH scoring",
        "audit_architecture": "Iterative generate → evaluate (CONCH) → critique (LLM + PubMed cross-reference) → refine prompts; pathologist-in-the-loop",
        "audit_tools": "CONCH VLM; PubMed knowledge cross-check",
        "audit_tasks": "Spatial pathway inference on WSI conditioned on bulk-RNA",
        "audit_paradigm": "In-context learning; human-in-the-loop",
        "audit_result": "Claude 4.0 best (cosine 0.7); validated against spatial-transcriptomic ground truth",
    ...
    "models": [
      {
        ...
        "audit_architecture": "Critique mechanism guided by a user-defined checklist; re-identifies informative regions via text-patch retrieval; iteratively enriches & reconciles the report",
        "audit_tasks": "Quality-controllable WSI report generation",
        "audit_paradigm": "Agentic critique-and-refine",
        "audit_result": "Controllable, high-coverage, evidence-grounded reports",
        "github": "",
        "hf": "",
  ...
  },
  {
    ...
    "models": [
      ...
      },
      {
        ...
        "audit_architecture": "Benchmark organized as a diagnostic tree linking nested regions across magnifications",
        "audit_tasks": "Evidence interpretation (I2T), verification (T2I), acquisition (localization), integration (multi-scale reasoning)",
        "audit_domain": "TCGA + private breast cohort",
        "audit_benchmark": "1,822 TCGA WSIs, 17,135 diagnostic paths (10 pathologists) + 190 private breast WSIs",
        "audit_result": "Evaluates 20 models; localization hard (mIoU <0.09); hit rate drops with magnification",
        "github": "",
        "hf": "",
    ...
    "models": [
      {
        ...
        "audit_architecture": "Survey mapping the shift from static analysis to autonomous diagnostic workflows; taxonomy of agent architectures & capabilities",
        "audit_tasks": "Literature survey of computational-pathology agents",
      ...
      },
      {
        ...
        "audit_architecture": "International expert perspective connecting deployable clinical relevance with technical maturity, operational readiness, and economic/regulatory context",
        "audit_tasks": "Perspective on responsible clinical integration of foundation/agentic AI",
